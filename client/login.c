 ...
 */
 //send info online
void send_info(){
    struct ChatMsg *chatmessage_connect;
    chatmessage_connect = (struct ChatMsg *)malloc(sizeof(struct ChatMsg));
    memset(chatmessage_connect, 0 , sizeof(struct ChatMsg));
    strcpy(chatmessage_connect->to_who,"all");
    strcpy(chatmessage_connect->who,u);
	chatmessage_connect->Flag = 400;
	strcpy(chatmessage_connect->Msg,strcat(u," is now online!"));
	send(socket_fd,chatmessage_connect, sizeof(struct ChatMsg),0);
}
void get_login_info(GtkWidget *widget,gpointer data){
